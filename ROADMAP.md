...
- **KM-c ✅** Popup düzeltmeleri (commit hash placeholder, 2026-05-04, atomik). İki patch tek dosyaya (`vehicle_popup.ts`):
  - **c.1** Mojibake guard — `isMojibake(long_name)` true ise bozuk metin gizlenir, `⚠ Hat adı okunamıyor` uyarısı görünür. Önceki davranış (⚠ + bozuk metin yan yana) Yağız 2026-05-04 smoke'da kafa karıştırıcı bulundu (Spec Ek A.19 borç #3).
  - **c.2** Metrobüs label — `VehicleProps.is_metrobus?: boolean` eklendi, `renderIettPopup` `is_metrobus===true` ise "Metrobüs", aksi halde eski "İETT Otobüs" (Spec Ek A.19 borç #8). Pipeline KM5-e.1 (backend) → snapshot_store → fleet_layer feature.properties → popup zincirinde sadece son halka eklendi; backend ve renderer dokunulmadı.
  - Vitest 17 case (önceki 13 + 4 yeni: mojibake metin gizleme, clean long_name regression, metrobüs label true/false/undefined). Frontend suite 244/244 (önceki 240 + 4). tsc temiz, vite build 1.06 MB / gzip 285 KB.
- **KM-d ✅** Ferry route focus + popup metadata fix (commit hash placeholder, 2026-05-04). Borç #6 + Yağız 2026-05-04 ek bulgu. Tanı raporu `_research/2026-05-04-km-d-ferry-doubleclick-diagnosis.md` hipotez (c) %90 kanıtladı; ek smoke'da popup metadata bug'ı keşfedildi → kapsam genişledi:
  - **KM-d.1** Ferry çift tıklama sessiz: ferry hatları `route_lines_layer.collection`'a eklenmiyor (KM3 paterni, denizüstü polyline anlamsız). `getRoutesBBox` ferry için her zaman null, `SnapshotStore.getVehicleBBoxForRoutes` İETT canlı kapsamında — vapur scheduled. Fix: `ScheduledFleet.getRouteBBox/getRoutesBBox` ekledi (active PreparedTrip polyline koordinatlarından union bbox), `main.ts:focusAndZoom` üçüncü fallback olarak tüm `scheduledFleets` üzerinde tara.
  - **KM-d.2** Vapur popup "Hat metadata bulunamadı": `loadAlwaysVisibleRoutes` sadece polyline modlar için `routeStore.registerSummaries(polylineSummaries)` çağırıyordu (main.ts:161). Ferry için ayrı çağrı yoktu → `RouteStore.summariesByRouteId` ferry route_id'lerini içermiyordu → `getMeta(props.route_id)` null → vehicle_popup "Hat metadata bulunamadı" fallback'ı. Fix: ferrySummaries için de `registerSummaries` çağrısı eklendi.
  - Vitest 254/254 (önceki 244 + 6 `ScheduledFleet.getRouteBBox/getRoutesBBox` + 4 `RouteStore.registerSummaries/getMeta`). tsc + vite build temiz, backend dokunulmadı.
- **KM-e ✅** Filter Reset state corruption fix (commit hash placeholder, 2026-05-04, yayın blokeleyici). Borç #7. Tanı raporu `_research/2026-05-04-km-e-filter-reset-corruption-diagnosis.md` hipotez (a) %45 + ek bulgu (d) %30. Browser smoke skip (Yağız 2026-05-04 kararı: kod okuması yeterince güçlü, KM-f tek smoke turu). İki fix atomik tek commit:
  - **Fix-B** fleet-circles iki-sistem filter çakışması: `applyFilters` (route_id-based) ve `applyFleetVisibilityFilter` (is_metrobus-based) aynı layer'a setFilter ediyordu, sonuncusu kazanıyordu → Reset sonrası `route_id`-null İETT vehicle'lar (KM5-a mapping retire) gizleniyordu. Düzeltme: `applyFilters` artık fleet-circles'a dokunmuyor; tek-kanal sahip `applyFleetVisibilityFilter`. Initial sync `applyFleetVisibilityFilter()` çağrısı eklendi (önce yoktu, fleet-circles ilk kez applyFilters tarafından setlenip sonra ezilirdi).
  - **Fix-A** setFilter chain RAF debounce: yeni `frame_debouncer.ts::debounceFrame` helper. `routeVisibility.subscribe` ve fleet visibility callback'leri debounce'lu — Reset/Tümü/Hiçbiri 1 fire = 2 setFilter (önceki 3'ten azaldı; fleet-circles ayrıştı), N hızlı toggle tek frame'e sıkışır. Hipotez (a) cumulative paint maliyeti azalır.
  - Vitest 258/258 (önceki 254 + 4 `frame_debouncer`: çoklu schedule → 1 fn run, frame sonrası reschedule, hiç schedule yoksa fn çalıştırılmaz, Reset/multi-toggle simülasyonu). tsc + vite build temiz, backend dokunulmadı. KM-f browser smoke turunda gerçek davranış doğrulanır.
- **KM-g ✅** Reset/Tümü/Hiçbiri full filter coverage (commit hash placeholder, 2026-05-05). Borç #12 + #13 + #14 (Spec Ek A.19) tek koordinasyon noktasıyla kapandı. Tasarım belgesi `_research/2026-05-05-km-g-filter-architecture-redesign.md` Tasarım A (minimal koordinasyon, ~70 satır tahmini, gerçekleşen +125 net satır — 90'ı test). KM-e sonrası filter mimarisi iki kanal kalmak zorunda (veri iki boyutlu: route_id + is_metrobus, mapping retire KM5-a load-bearing); problem buton kapsamının dar olmasıydı — Tümü/Hiçbiri/Reset sadece Kanal A'ya (`route_id setBulkVisible/resetToDefault`) dokunuyordu, Kanal B (bus/metrobüs filter expression) ve Kanal C (routeFocus paint) ortagonal kalıyordu.
  - **Kanal A** dokunulmadı (mevcut `setBulkVisible/resetToDefault` davranışı korundu).
  - **Kanal B** entegrasyonu (Borç #12/#14): `route_panel.ts`'e `onSelectAllChange?:(allOn)=>void` + `onResetRequested?:()=>void` opt callback'leri. `main.ts`'te `applyFleetState(bus, metrobus)` helper — `busVisible/metrobusVisible` mutate, `debouncedApplyFleet` (Fix-A korunur), `panel.setFleetVisibility({bus, metrobus})` ile UI checkbox state sync. Hiçbiri sonrası "M3 üstünde sarı nokta" (aslında M3'e yakın İETT bus, fleet-circles paint sarı) ve metrobüs noktaları kapanır.
  - **Kanal C** entegrasyonu (Borç #13): aynı `applyFleetState` `routeFocus.setFocus(null)` çağırır — sticky focus → fleet-circles 0.2 opacity "hayalet leke" (~6900 vehicle, M5 koridor sonu yoğun) global aksiyondan sonra temizlenir. Üç buton da focus reset eder (Yağız onayı: global aksiyon → temiz başlangıç).
  - **API yüzeyi:** `RoutePanelHandle.setFleetVisibility(state)` public metod (atomik tek update, callback tetiklemez — sonsuz döngü guard). Buton etiketleri "Tümü/Hiçbiri/Reset" dokunulmadı (TR netliği v0.8.2 polish).
  - **Bonus tarama:** fleet-circles paint expression'da `circle-opacity`'yi etkileyen state taraması (`buildFleetPaint(focused)` tek noktası — `routeFocus.subscribe(applyFocusPaint)` ile bağlı). Snapshot reducer (`updateFleet`) sadece geometri/properties yazıyor, paint'e dokunmuyor. `applyFleetVisibilityFilter` sadece `setFilter`. **Sonuç: routeFocus dışında fleet-circles opacity'sini etkileyen başka state yok.** Kapsam genişletmesi gerekmedi.
  - Vitest 266/266 (önceki 258 + 8 yeni KM-g case: Tümü → onSelectAllChange(true), Hiçbiri → onSelectAllChange(false), Reset → onResetRequested, Reset orthogonal — onSelectAllChange tetiklemez, setFleetVisibility false/true/asymmetric checkbox sync, setFleetVisibility loop guard). tsc + vite build temiz, backend dokunulmadı (175 + 44 = 219 frozen).
